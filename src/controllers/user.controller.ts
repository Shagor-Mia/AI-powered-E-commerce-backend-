...
import { Request, Response } from "express";
import userModel, { userUpdateSchema } from "../models/user.model";
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (req.user?.role === UserRole.SELLER) {
      const users = await userModel.find({
        $and: [
          { role: { $in: [UserRole.SELLER, UserRole.USER] } },
          { isDeleted: false },
        ],
      });
      sendSuccess(res, "Users fetched successfully", users);
...
// Get single user by ID with access rules
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (req.user?.role === UserRole.SELLER) {
      ...
      if (user) {
        ...
        sendError(res, "User not found or access denied", 404);
      }
      return;
    }
...
// Update user with Zod validation (admin only)
